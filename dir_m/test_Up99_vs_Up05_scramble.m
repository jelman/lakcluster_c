...
if (flag_verbose); disp(sprintf(' %% numel(intersect(dataset_{1+ndataset_Up05}.bim_vid_(1+efind(tmp_mc_)),dataset_{1+ndataset_Up05}.bim_vid_)): %d/%d',numel(intersect(dataset_{1+ndataset_Up05}.bim_vid_(1+efind(Up05_bim_ADp_<=0.05)),dataset_{1+ndataset_Up05}.bim_vid_)),numel(unique(dataset_{1+ndataset_Up05}.bim_vid_)))); end;

flag_calc = 1;
if flag_calc;
%%%%%%%%;
% Now run a simple biclustering for the Up05 data-set. ;
%%%%%%%%;
str_lak_vs_dex = 'dex';
...
parameter_Up05.str_mc_0in = str_mc_0in;
...
%%%%%%%%;
tmp_niteration = 30; %<-- this is the iteration index (zero-based) at which you want to define the bicluster. ;
...
parameter_Up05_scramble_BC0.n_shuffle = 4;
...
% Plot results within p-val mask;
...
plot(trace_Up05__.niter_s0000_(pval_mask),ZR_scramble_BC0_(pval_mask),'g-','LineWidth',linewidth_big);
hold off;
xlim([0,max(trace_Up05__.niter_s0000_(pval_mask))+1]); xlabel('niteration'); ylim([-10,+10]); ylabel('Z');
subplot(1,2,2);
hold on;
...
plot(tmp_r_eli_(pval_mask)/ncases,ZR_scramble_BC0_(pval_mask),'g-','LineWidth',linewidth_big);
hold off;
