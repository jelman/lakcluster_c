...
%%%%%%%%;
% Now pick out an iteration and calculate pca directly. ;
%%%%%%%%;
tmp_parameter_trn = parameter_trn;
pca_rank = pca_rank_use;
mr_A_rem_trn_ = 0.0*mr_A_alt_trn_; mr_A_rem_trn_(1+index_r_rem_trn_) = 1;
assert(sum(mr_A_rem_trn_)==sum(mr_A_rem_trn_.*mr_A_alt_trn_));
mr_Z_rem_trn_ = mr_Z_alt_trn_;
...
mr_dvx_trnUp05_tstAp05_nix_(1+efind(mr_Z_alt_trn_)) = 1;
mr_dvx_trnUp05_tstAp05_nix_(1+efind(mr_A_alt_trn_)) = 2;
mr_dvx_trnUp05_tstAp05_nix_(1+efind(mr_A_rem_trn_)) = 3;
if flag_disp;
...
% this time excluding bc0 cases and snps. ;
%%%%%%%%;
dir_trunk_Up05 = sprintf('%s/dir_Up05',dir_trunk);
ncontinent = 0;
if (flag_verbose); disp(sprintf(' %% ;')); end;
...
mr_A_alt_0b_(1+index_r_rem_trn_) = 0;
mr_Z_alt_0b_ = mr_Z_ori_0b_.*mr_Up05_p01_continent_pc__(:,1+ncontinent);
mr_Z_alt_0b_(1+index_r_rem_trn_) = 0;
mc_A_alt_0b_ = mc_A_ori_0b_; 
mc_A_alt_0b_(1+index_c_rem_trn_) = 0;
...
if flag_calc;
%%%%%%%%;
% Now calculate many _D_ principal-components across the entire Up05 data-set. ;
%%%%%%%%;
str_lak_vs_dex = 'dex';
str_prefix = 'test2mds_maf01';
gamma = 0.05;
...
%{
%%%%%%%%;
% Now check to see if the two biclusters from ncontinent==0 and ncontinent==1 are similar. ;
%%%%%%%%;
tmp_dir = '/home/rangan/dir_bcc/dir_jelman/dir_Up05/dir_test2mds_maf01_analyze';
tmp_c0_bim_vid_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent0_D_m2r1_g050/test2mds_maf01_ni23_bim_vid_.txt',tmp_dir);
tmp_c0_bim_name_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent0_D_m2r1_g050/test2mds_maf01_ni23_bim_name_.txt',tmp_dir);
tmp_c0_fam_iid_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent0_D_m2r1_g050/test2mds_maf01_ni23_fam_iid_.txt',tmp_dir);
tmp_c1_bim_vid_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent1_D_m2r1_g050/test2mds_maf01_ni20_bim_vid_.txt',tmp_dir);
tmp_c1_bim_name_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent1_D_m2r1_g050/test2mds_maf01_ni20_bim_name_.txt',tmp_dir);
tmp_c1_fam_iid_ = sprintf('%s/dir_test2mds_maf01_dex_p01_continent1_D_m2r1_g050/test2mds_maf01_ni20_fam_iid_.txt',tmp_dir);
%%%%;
if ~exist(tmp_c0_bim_vid_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c0_bim_vid_)); end;
if ~exist(tmp_c0_bim_name_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c0_bim_name_)); end;
if ~exist(tmp_c0_fam_iid_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c0_fam_iid_)); end;
if ~exist(tmp_c1_bim_vid_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c1_bim_vid_)); end;
if ~exist(tmp_c1_bim_name_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c1_bim_name_)); end;
if ~exist(tmp_c1_fam_iid_,'file'); disp(sprintf(' %% Warning, %s not found',tmp_c1_fam_iid_)); end;
%%%%;
tmp_fp=fopen(tmp_c0_bim_vid_,'r'); c0_bim_vid_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c0_bim_vid_ = c0_bim_vid_{1};
tmp_fp=fopen(tmp_c0_bim_name_,'r'); c0_bim_name_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c0_bim_name_ = c0_bim_name_{1};
tmp_fp=fopen(tmp_c0_fam_iid_,'r'); c0_fam_iid_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c0_fam_iid_ = c0_fam_iid_{1};
tmp_fp=fopen(tmp_c1_bim_vid_,'r'); c1_bim_vid_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c1_bim_vid_ = c1_bim_vid_{1};
tmp_fp=fopen(tmp_c1_bim_name_,'r'); c1_bim_name_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c1_bim_name_ = c1_bim_name_{1};
tmp_fp=fopen(tmp_c1_fam_iid_,'r'); c1_fam_iid_ = textscan(tmp_fp,'%s'); fclose(tmp_fp); c1_fam_iid_ = c1_fam_iid_{1};
%%%%;
 tmp_n_snp = dataset_{1+ndataset_Up05}.n_snp;
 c0_n_snp = numel(c0_bim_name_);
 c1_n_snp = numel(c1_bim_name_);
 cap_n_snp_emp = numel(intersect(c0_bim_name_,c1_bim_name_));
 cap_n_snp_est = ceil((c0_n_snp/tmp_n_snp)*(c1_n_snp/tmp_n_snp)*tmp_n_snp);
 disp(sprintf(' %% c0_bim_name_ %d, c0_bim_name_ %d, intersection %d vs %d (predicted)',c0_n_snp,c1_n_snp,cap_n_snp_emp,cap_n_snp_est));
%}
