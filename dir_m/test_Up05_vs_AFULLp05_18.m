...
% index_AFULLp05_from_cap_ cross-references the allele_cap and the AFULLp05 dataset. ;
% index_Up05_setminus_cap_ lists those allele-combinations in Up05 which are not part of the intersection. ;
% index_AFULLp05_setminus_cap_ lists those allele-combinations in AFULLp05 which are not part of the intersection. ;
%%%%%%%%;
ndataset_Up05 = 0;
ndataset_AFULLp05 = 1;
n_patient_Up05 = dataset_{1+ndataset_Up05}.n_patient;
n_patient_AFULLp05 = dataset_{1+ndataset_AFULLp05}.n_patient;
...
if (flag_verbose); disp(sprintf(' %% numel(index_AFULLp05_from_cap_): %d',numel(index_AFULLp05_from_cap_))); end;
if (flag_verbose); disp(sprintf(' %% numel(index_Up05_setminus_cap_): %d',numel(index_Up05_setminus_cap_))); end;
if (flag_verbose); disp(sprintf(' %% numel(index_AFULLp05_setminus_cap_): %d',numel(index_AFULLp05_setminus_cap_))); end;
...
%%%%%%%%;
% Over the course of the next two paragraphs we define Up05_A_p_ and AFULLp05_A_p_. ;
...
pca_str_infix_tst='D_trnUp05_tstAFULLp05_nix_p01';
%%%%;
...
%%%%%%%%;
ndataset=ndataset_AFULLp05;
% Assign subjects with pheno value 3 to case array ;
dataset_{1+ndataset}.mx__.mr_A_full_(dataset_{1+ndataset}.fam_dvx_==3) = 1 ;
mx_tst__ = dataset_{1+ndataset}.mx__;
tmp_parameter_tst = dataset_{1+ndataset}.parameter;
if (flag_verbose); disp(sprintf(' %% tmp_parameter_tst.dir_0in: %s',tmp_parameter_tst.dir_0in)); end;
if (flag_verbose); disp(sprintf(' %% tmp_parameter_tst.str_prefix: %s',tmp_parameter_tst.str_prefix)); end;
...
mr_Z_ori_tst_ = mx_tst__.mr_Z_full_;
mc_A_ori_tst_ = mx_tst__.mc_A_;
pca_str_infix_tst='D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01';
%%%%;
parameter_A_p_p01 = tmp_parameter_tst;
pca_A_p_str_infix = 'full';
parameter_A_p_p01.str_name_s0000 = 'A_p_p01';
...
tmp_AFULLp05_A_p_p01_ = mda_read_r8(str_AFULLp05_A_p_p01); 
if (flag_disp>1);
figure(1+nf);nf=nf+1;clf;figsml;
plot(tmp_AFULLp05_A_p_p01_);
xlabel('pcol');ylabel('A_p','Interpreter','none');
...
n_continent = max(mr_Up05_p01_continent_)+1; %<-- n_continent is 1-based.
...
%%%%%%%%;
if (flag_verbose); disp(sprintf(' %% ')); end;
if (flag_verbose); disp(sprintf(' %% We divide AFULLp05 data into three continents. ;')); end;
mr_AFULLp05_p01_continent_ = textread(sprintf('%s/dir_AFULLp05/mr_AFULLp05_from_Up05_cap_AFULLp05_p01_continent_.txt',dir_trunk));
n_continent = max(mr_AFULLp05_p01_continent_)+1; %<-- n_continent is 1-based.
mr_AFULLp05_p01_continent_pc__ = zeros(n_patient_AFULLp05,n_continent);
for ncontinent=0:n_continent-1; %<-- n_continent is 1-based, ncontinent is 0-based.
mr_AFULLp05_p01_continent_pc__(:,1+ncontinent) = (mr_AFULLp05_p01_continent_==ncontinent);
end;%for ncontinent=0:n_continent-1;
%%%%%%%%;
...
mr_A_alt_trn_ = mr_A_ori_trn_.*mr_Up05_p01_continent_pc__(:,ncontinent);%<-- if continent labels are 0-based, change to 1+ncontinent ;
mr_Z_alt_trn_ = mr_Z_ori_trn_.*mr_Up05_p01_continent_pc__(:,ncontinent);%<-- if continent labels are 0-based, change to 1+ncontinent ;
...
%%%%%%%;
% Find iteration with max Z-value to define bicluster ;
%%%%%%%;
% Load trace, exclude mask for Z-max (exclude biclusters with <5% or >95%
% of case population);
trace_trn____ = load_trace__from_dir_ver0(parameter_trn.dir_out_trace);
tmp_r_eli_ = trace_trn____.r_rem_s0000_(1+0) - trace_trn____.r_rem_s0000_ ;
ncases = trace_trn____.r_rem_s0000_(1+0);
pval_mask = trace_trn____.r_rem_s0000_/ncases > .05;
Z_max_mask = (trace_trn____.r_rem_s0000_/ncases > .05) & (trace_trn____.r_rem_s0000_/ncases < .95);
% Calculate p-value for trace ;
parameter_trn.flag_verbose = 1; 
parameter_trn.niteration_alo = 1;
parameter_trn.niteration_ahi = max(trace_trn____.niter_s0000_(pval_mask))+1;
[~,p_all_out,p_avg_out,p_top_out] = p_from_trace__ver0(parameter_trn,trace_trn____);
% Now run Z_imax_zerobased to pick out an internal iteration of interest. ;
tmp_Z_ = trace_trn____.ZR_s0000_(Z_max_mask); %<-- this is the z-score for the traces of the original data (i.e., nshuffle==0). ;
...
tmp_nlpR = tmp_Z_max;
tmp_ij_nlpR = tmp_Z_max_index+1; %<-- Get 1-based index 
%%%%;
...
% defined above (i.e., the allele-combination-intersection of Up05 and AFULLp05). ;
%%%%%%%%;
...
%%%%%%%%;
pca_str_infix_trn=sprintf('trnUp05_tstAFULLp05_ni%d_p01',tmp_ij_nlpR-1);
tmp_parameter_trn.flag_force_create = 0;
parameter_D_trnUp05_tstAFULLp05_nix_p01 = tmp_parameter_trn;
parameter_D_trnUp05_tstAFULLp05_nix_p01.str_A_p = str_Up05_A_p_p01;
parameter_D_trnUp05_tstAFULLp05_nix_p01.str_name_s0000 = 'pca_D_trnUp05_tstAFULLp05_nix_p01';
[ ...
 parameter_D_trnUp05_tstAFULLp05_nix_p01 ...
,AZnV_D_trnUp05_tstAFULLp05_nix_p01_ ...
,AnV_D_trnUp05_tstAFULLp05_nix_p01_ ...
,ZnV_D_trnUp05_tstAFULLp05_nix_p01_ ...
,V_D_trnUp05_tstAFULLp05_nix_p01_ ...
] = ...
...
%%%%;
mr_dvx_trnUp05_tstAFULLp05_nix_ = 0.0*mr_A_ori_trn_;
mr_dvx_trnUp05_tstAFULLp05_nix_(1+efind(mr_Z_alt_trn_)) = 1; %<-- color ctrl-patients. ;
mr_dvx_trnUp05_tstAFULLp05_nix_(1+efind(mr_A_alt_trn_)) = 2; %<-- color case-patients. ;
mr_dvx_trnUp05_tstAFULLp05_nix_(1+efind(mr_A_rem_trn_)) = 3; %<-- color bicluster-patients (a subset of cases). ;
if flag_disp;
figure(1+nf);nf=nf+1;clf;figsml;fig80s;
markersize_sml = 12;
markersize_big = 16;
hold on;
tmp_index_ = efind( (mr_dvx_trnUp05_tstAFULLp05_nix_==1) | (mr_dvx_trnUp05_tstAFULLp05_nix_==2) );
scatter(AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+0),AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+1),markersize_sml,mr_dvx_trnUp05_tstAFULLp05_nix_(1+tmp_index_),'filled','MarkerEdgeColor','k');
tmp_index_ = efind( (mr_dvx_trnUp05_tstAFULLp05_nix_==3) );
scatter(AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+0),AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+1),markersize_big,mr_dvx_trnUp05_tstAFULLp05_nix_(1+tmp_index_),'filled','MarkerEdgeColor',0.85*[1,1,1]);
axis equal; grid on;
title('AZnV_D_trnUp05_tstAFULLp05_nix_p01_','Interpreter','none'); xlabel('PC1'); ylabel('PC2');
end;%if flag_disp;
%%%%;
...
% mr_A_alt_0b_ = mr_A_ori_0b_.*mr_Up05_p01_continent_pc__(:,ncontinent); %<-- Edit to 1+ncontinent if 0-based;
% mr_A_alt_0b_(1+index_r_rem_trn_) = 0; %<-- Here we explicitly exclude the case-rows in the first bicluster. ;
% mr_Z_alt_0b_ = mr_Z_ori_0b_.*mr_Up05_p01_continent_pc__(:,ncontinent); %<-- Edit to 1+ncontinent if 0-based;
...
axis equal; axis vis3d;
%%%%;
fname_fig = sprintf('/%s/dir_Up05/Up05_D_p01_FIGA',dir_trunk);
...
xlim_ = [min(x_),max(x_)];
tmp_h_Z_ = hist(AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_Z_alt_trn_),1+0),x_);
tmp_h_Z_ = tmp_h_Z_/sum(tmp_h_Z_);
tmp_h_A_ = hist(AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_A_alt_trn_),1+0),x_);
...
% We store this in: ;
% D_trnUp05_tstAFULLp05_nix_p01_auc_t_, auc values. ;
% D_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_, log p-values for the auc. ;
% D_trnUp05_tstAFULLp05_nix_p01_thr_t_, threshold values. ;
%%%%%%%%
tmp_Z_ = AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_Z_alt_trn_),1+0);
tmp_A_ = AZnV_D_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_A_alt_trn_),1+0);
tmp_threshold_ = sort([tmp_Z_;tmp_A_],'ascend'); tmp_n_threshold = numel(tmp_threshold_);
D_trnUp05_tstAFULLp05_nix_p01_auc_t_ = zeros(tmp_n_threshold,1);
D_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_ = zeros(tmp_n_threshold,1);
...
D_trnUp05_tstAFULLp05_nix_p01_auc_t_(1+tmp_nthreshold) = tmp_auc;
D_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_(1+tmp_nthreshold) = tmp_logp_auc;
end;%for tmp_nthreshold=0:tmp_n_threshold-1;
D_trnUp05_tstAFULLp05_nix_p01_thr_t_ = tmp_threshold_;

%%%%%%%%;
% Now project AFULLp05 data onto the principal-components defined using the bicluster. ;
...
%%%%%%%%;
pca_str_infix_trn=sprintf('trnUp05_tstAFULLp05_ni%d_p01',tmp_ij_nlpR-1); %<-- this defines the pca file to use. ;
parameter_D_trnUp05_tstAFULLp05_nix_p01.str_V_ = sprintf('%s/dir_pca/dir_pca_mda/pca_D_%s_k%d_B44_V_.mda',parameter_D_trnUp05_tstAFULLp05_nix_p01.dir_out_s0000,pca_str_infix_trn,pca_rank);
if ~exist(parameter_D_trnUp05_tstAFULLp05_nix_p01.str_V_,'file');
disp(sprintf(' %% Warning, %s not found',parameter_D_trnUp05_tstAFULLp05_nix_p01.str_V_));
end;%if ~exist(parameter_D_trnUp05_tstAFULLp05_nix_p01.str_V_,'file');
D_trnUp05_tstAFULLp05_nix_p01_V_ = mda_read_r8(parameter_D_trnUp05_tstAFULLp05_nix_p01.str_V_);
if (flag_verbose); disp(sprintf(' %% size of intersection: %d',numel(index_Up05_from_cap_))); end;
if (flag_verbose); disp(sprintf(' %% Support of V_: %d',sum(abs(D_trnUp05_tstAFULLp05_nix_p01_V_(:,1))>0))); end;
if (flag_verbose); disp(sprintf(' %% intersection of supp(V_) with cap: %d',numel(intersect(index_Up05_from_cap_,efind(abs(D_trnUp05_tstAFULLp05_nix_p01_V_(:,1))>0))))); end;
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_V_ = ij_AFULLp05_from_Up05__*D_trnUp05_tstAFULLp05_nix_p01_V_;
if (flag_verbose); disp(sprintf(' %% fnorm(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_V_(ij_AFULLp05_from_cap_,:) - D_trnUp05_tstAFULLp05_nix_p01_V_(ij_Up05_from_cap_,:)): %0.16f',fnorm(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_V_(ij_AFULLp05_from_cap_,:) - D_trnUp05_tstAFULLp05_nix_p01_V_(ij_Up05_from_cap_,:)))); end;

%%%%%%%%;
% Now project AFULLp05 data onto D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_V_. ;
%%%%%%%%;
ndataset=ndataset_AFULLp05;
mx_tst__ = dataset_{1+ndataset}.mx__;
tmp_parameter_tst = dataset_{1+ndataset}.parameter;
...
mr_Z_ori_tst_ = mx_tst__.mr_Z_full_;
mc_A_ori_tst_ = mx_tst__.mc_A_;
assert(numel(mr_A_ori_tst_)==numel(mr_AFULLp05_p01_continent_));
mr_A_alt_tst_ = mr_A_ori_tst_.*mr_AFULLp05_p01_continent_pc__(:,ncontinent);
mr_Z_alt_tst_ = mr_Z_ori_tst_.*mr_AFULLp05_p01_continent_pc__(:,ncontinent);
mc_A_alt_tst_ = mc_A_ori_tst_;
pca_mr_A_tst_ = { mr_A_alt_tst_ };
pca_mr_Z_tst_ = { mr_Z_alt_tst_ };
pca_mc_A_tst = mc_A_alt_tst_; pca_mc_A_tst(1+index_AFULLp05_setminus_cap_) = 0;
pca_str_infix_tst='D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01';
parameter_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01 = tmp_parameter_tst;
parameter_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01.str_A_p = str_AFULLp05_A_p_p01;
parameter_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01.str_name_s0000 = ...
  sprintf('pca_D_AFULLp05c%d_from_trnUp05c%d_tstAFULLp05c%d_nix_p01',ncontinent,ncontinent,ncontinent);
[ ...
 parameter_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01 ...
,AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_ ...
,AnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_ ...
,ZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_ ...
,tmp_V_ ...
] = ...
xxxcluster_fromdisk_uADZSZDA_pca_proj_from_mx_ver16( ...
 parameter_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01 ...
,pca_rank ...
,D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_V_ ...
,pca_mr_A_tst_ ...
,pca_mr_Z_tst_ ...
,pca_mc_A_tst ...
,pca_str_infix_tst ...
...
%%%%;
mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_ = 0.0*mr_A_ori_tst_;
mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_(1+efind(mr_Z_alt_tst_)) = 1;
mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_(1+efind(mr_A_alt_tst_)) = 2;
figure(1+nf);nf=nf+1;clf;figsml;fig80s;
markersize_use = 12;
tmp_index_ = efind(mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_);
scatter(AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+0),AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+tmp_index_,1+1),markersize_use,mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_(1+tmp_index_),'filled','MarkerEdgeColor','k');
axis equal; grid on;
title('AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_','Interpreter','none'); xlabel('PC1'); ylabel('PC2');
%%%%;
...
% projected onto the dominant principal-component of the bicluster. ;
% The ctrls are shown in blue. ;
% Note that this projection is limited to the testing-set (AFULLp05). ;
%%%%%%%%;
x_ = transpose(linspace(-60,+80,128)); 
xlim_ = [min(x_),max(x_)];
tmp_h_Z_ = hist(AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_Z_alt_tst_),1+0),x_);
tmp_h_Z_ = tmp_h_Z_/sum(tmp_h_Z_);
tmp_h_A_ = hist(AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_A_alt_tst_),1+0),x_);
tmp_h_A_ = tmp_h_A_/sum(tmp_h_A_);
figure(1+nf);nf=nf+1;figsml;
hold on;
stairs(x_,tmp_h_Z_,'b-');
...
%%%%%%%%;
% Now, for the testing dataset AFULLp05, ;
% We define the 'projected value' of each patient as the ;
% position of that patient after projection onto the ;
% dominant pc of the first bicluster ;
% (defined using only the allele-combinations in the bicluster which lie within allele_cap). ;
...
% for those patients above a certain threshold of this projected value. ;
% We store this in: ;
% D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_auc_t_, auc values. ;
% D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_, log p-values for the auc. ;
% D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_, threshold values. ;
%%%%%%%%
tmp_Z_ = AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_Z_alt_tst_),1+0);
tmp_A_ = AZnV_D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_(1+efind(mr_A_alt_tst_),1+0);
tmp_threshold_ = sort([tmp_Z_;tmp_A_],'ascend'); tmp_n_threshold = numel(tmp_threshold_);
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_auc_t_ = zeros(tmp_n_threshold,1);
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_ = zeros(tmp_n_threshold,1);
for tmp_nthreshold=0:tmp_n_threshold-1;
tmp_threshold = tmp_threshold_(1+tmp_nthreshold);
tmp_Z_thr_ = tmp_Z_(1+efind(tmp_Z_>=tmp_threshold));
tmp_A_thr_ = tmp_A_(1+efind(tmp_A_>=tmp_threshold));
...
tmp_logp_auc = logp_auc_0(tmp_auc,tmp_n);
if (flag_verbose>1); disp(sprintf(' %% thr %0.2f: tmp_auc: %0.3f <-- nlp %0.2f <-- p %0.6f',tmp_threshold,tmp_auc,-tmp_logp_auc,exp(tmp_logp_auc))); end;
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_auc_t_(1+tmp_nthreshold) = tmp_auc;
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_(1+tmp_nthreshold) = tmp_logp_auc;
end;%for tmp_nthreshold=0:tmp_n_threshold-1;
D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_ = tmp_threshold_;
...
% while the testing-data (AFULLp05) is shown in green. ;
...
hold on;
ymax = 10;
tmp_dlim_0_ = [min(D_trnUp05_tstAFULLp05_nix_p01_thr_t_);max(D_trnUp05_tstAFULLp05_nix_p01_thr_t_)];
tmp_dlim_1_ = [min(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_);max(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_)];
tmp_dlim_ = [min([tmp_dlim_0_;tmp_dlim_1_]),max([tmp_dlim_0_;tmp_dlim_1_])];
stairs(D_trnUp05_tstAFULLp05_nix_p01_thr_t_,min(10,-D_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_),'r-','LineWidth',2);
stairs(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_,min(10,-D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_logp_auc_t_),'g-','LineWidth',2);
%stairs(D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_thr_t_,min(10,10*D_AFULLp05_from_trnUp05_tstAFULLp05_nix_p01_auc_t_),'-','Color',0.85*[1,1,1],'LineWidth',2);
...
title('AZnV_D_trnUp05_tstAFULLp05_nix_p01_','Interpreter','none');
fname_fig_pre = sprintf('%s/test_Up05_vs_AFULLp05_18b_FIGA',dir_jpg); %<-- note that this matches the previous test_Up05_vs_AFULLp05_18b.m ;
...
fname_mat = sprintf('%s/trnUp05_tst_AFULLp05_ncontinent%d.mat',dir_mat_replication,ncontinent);
...
save(fname_mat ...
     ...
     ,'mr_Z_alt_trn_' ...
     ,'mr_Z_alt_tst_' ...
     ,'mr_dvx_AFULLp05_from_trnUp05_tstAFULLp05_nix_' ...
     ,'mr_dvx_trnUp05_tstAFULLp05_nix_' ...
     );
